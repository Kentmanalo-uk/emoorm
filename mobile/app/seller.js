...
function Insights() {
  ...
  const [ratingStats, setRatingStats] = useState(initialData?.ratingStats || { averageRating: 0, totalReviews: 0 });
  const [loading, setLoading] = useState(!initialData);
  const [replyOpenId, setReplyOpenId] = useState('');
  const [replyDraft, setReplyDraft] = useState('');
  const [savingReplyId, setSavingReplyId] = useState('');
  useEffect(() => { const fresh = getCachedData(SELLER_CACHE.insights, SELLER_CACHE_TTL); if (fresh) { setAnalytics(fresh.analytics); setReviews(fresh.reviews); setRatingStats(fresh.ratingStats || { averageRating: 0, totalReviews: 0 }); setLoading(false); return; } refreshCachedData(SELLER_CACHE.insights, async () => { const [analyticsResponse, reviewsResponse] = await Promise.all([apiClient.get(ENDPOINTS.SELLER.ANALYTICS), apiClient.get(ENDPOINTS.REVIEWS.SELLER_MINE, { params: { pageSize: 10 } })]); return { analytics: analyticsResponse.data, reviews: reviewsResponse.data || [], ratingStats: reviewsResponse.ratingStats || { averageRating: 0, totalReviews: 0 } }; }).then((data) => { setAnalytics(data.analytics); setReviews(data.reviews); setRatingStats(data.ratingStats); }).catch((error) => toast.error('Failed to load insights', error.message)).finally(() => setLoading(false)); }, []);
  const startReply = (review) => { setReplyOpenId(review.id); setReplyDraft(review.sellerReply || ''); };
  const submitReply = async (review) => {
    const text = replyDraft.trim();
    if (!text) { toast.error('Reply cannot be empty'); return; }
    setSavingReplyId(review.id);
    try {
      const response = await apiClient.post(ENDPOINTS.REVIEWS.REPLY(review.id), { reply: text });
      setReviews((current) => { const next = current.map((item) => item.id === review.id ? { ...item, ...response.data } : item); setCachedData(SELLER_CACHE.insights, { analytics, reviews: next, ratingStats }); return next; });
      setReplyOpenId('');
      toast.success('Reply posted');
    } catch (error) { toast.error('Could not post reply', error.message); }
    finally { setSavingReplyId(''); }
  };
  if (loading) return <SellerOverviewSkeleton />;
  const maxRevenue = Math.max(1, ...(analytics?.salesByDay || []).map((day) => Number(day.total || 0)));
  return <ScrollView contentContainerStyle={styles.content}><View style={styles.kpiGrid}><Kpi label="Revenue" value={peso(analytics?.kpis?.revenue?.value)} /><Kpi label="Orders" value={analytics?.kpis?.orders?.value || 0} /><Kpi label="Customers" value={analytics?.kpis?.buyers?.value || 0} /><Kpi label="Rating" value={ratingStats.totalReviews ? Number(ratingStats.averageRating).toFixed(1) : 'New'} /></View><View style={styles.section}><Text style={styles.sectionTitle}>Daily Sales</Text><View style={styles.chart}>{(analytics?.salesByDay || []).slice(-14).map((day) => <View key={day.date} style={styles.barColumn}><View style={[styles.bar, { height: Math.max(3, (Number(day.total || 0) / maxRevenue) * 110) }]} /><Text style={styles.barLabel}>{new Date(day.date).getDate()}</Text></View>)}</View></View><View style={styles.section}><Text style={styles.sectionTitle}>Best Sellers</Text>{(analytics?.topProducts || []).slice(0, 5).map((product, index) => <View key={product.id} style={styles.row}><Text style={styles.rank}>{index + 1}</Text><View style={styles.rowBody}><Text style={styles.rowTitle}>{product.name}</Text><Text style={styles.rowMeta}>{product.quantity} sold</Text></View><Text style={styles.totalText}>{peso(product.revenue)}</Text></View>)}</View><View style={styles.section}><Text style={styles.sectionTitle}>Recent Reviews</Text>{reviews.length ? reviews.slice(0, 10).map((review) => <View key={review.id} style={styles.review}><View style={styles.reviewHeader}><Text style={styles.rowTitle}>{review.user?.fullName || 'Buyer'}</Text><StarRating rating={review.rating} size={13} /></View>{review.product?.name ? <Text style={styles.rowMeta}>on {review.product.name}</Text> : null}{review.comment ? <Text style={styles.reviewComment}>{review.comment}</Text> : null}{review.sellerReply && replyOpenId !== review.id ? <View style={styles.sellerReplyBox}><Text style={styles.sellerReplyLabel}>Your reply</Text><Text style={styles.reviewComment}>{review.sellerReply}</Text><Pressable accessibilityRole="button" accessibilityLabel="Edit reply" onPress={() => startReply(review)}><Text style={styles.replyLink}>Edit reply</Text></Pressable></View> : replyOpenId === review.id ? <View style={styles.replyForm}><TextField placeholder="Write a public reply…" value={replyDraft} onChangeText={setReplyDraft} multiline /><View style={styles.replyActions}><Pressable accessibilityRole="button" accessibilityLabel="Cancel reply" onPress={() => setReplyOpenId('')}><Text style={styles.replyCancelText}>Cancel</Text></Pressable><Pressable accessibilityRole="button" accessibilityLabel="Post reply" disabled={savingReplyId === review.id} style={styles.replySaveButton} onPress={() => submitReply(review)}>{savingReplyId === review.id ? <ActivityIndicator size="small" color={colors.white} /> : <Text style={styles.replySaveText}>Post Reply</Text>}</Pressable></View></View> : <Pressable accessibilityRole="button" accessibilityLabel="Reply to review" onPress={() => startReply(review)}><Text style={styles.replyLink}>Reply</Text></Pressable>}</View>) : <Text style={styles.emptyText}>No reviews yet.</Text>}</View></ScrollView>;
}
...
const styles = StyleSheet.create({
  ...
  sellerReplyBox: { marginTop: spacing.xs, padding: spacing.sm, borderRadius: radius.md, backgroundColor: colors.successLight || '#f0fdf4', gap: 2 },
  sellerReplyLabel: { fontSize: 11, fontWeight: '700', textTransform: 'uppercase', letterSpacing: 0.3, color: colors.success || '#047857' },
  replyLink: { marginTop: spacing.xs, fontSize: 12.5, fontWeight: '600', color: colors.primary },
  ...
  replyCancelText: { fontSize: 13, color: colors.textMuted, fontWeight: '600' },
